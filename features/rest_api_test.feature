...
Функция: REST API
  ...
  Сценарий: Работа с пользователями через REST API
    ...
    Тогда добавляю пользователя c логином t.simka именем vasy фамилией petrov паролем Qwerty123@
    И нахожу пользователя с логином t.simka
    И изменяю доступных параметров пользователя по логину t.simka на имя pety, фамилию ivanov, пароль smile123
    Тогда удаляю пользователя с логином t.simka
