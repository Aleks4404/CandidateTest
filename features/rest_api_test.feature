...
Функция: REST API
  ...
  Сценарий: Работа с пользователями через REST API
    ...
    Тогда добавляю пользователя c логином t.sima именем vasy фамилией petrov паролем Qwerty123@
    И нахожу пользователя с логином t.sima
    И изменяю доступных параметров пользователя по логину t.sima на имя pety, фамилию ivanov, пароль smile123
    Тогда удаляю пользователя с логином t.sima
